-- ameOs v46.2 [TOPBAR PATH & FS SAFETY FIX]
...
local function drawTopBar()
    ...
    -- ФИКС: Путь теперь всегда в верхнем баре
    local header = "ameOs | " .. activeTab
    if activeTab == "FILE" then header = header .. ": " .. currentPath end
    topWin.write(header:sub(1, w-8))
...
local function drawUI()
    ...
    elseif activeTab == "FILE" then
        -- Список файлов начинается сразу, т.к. путь уехал в TopBar
...
local function showContext(mx, my, file)
    ...
    if cx >= mx and cx < mx+12 and cy >= my and cy < my+#opts then
        ...
        -- ФИКС: Проверка fs.exists перед действиями
        if choice == "New File" then 
            mainWin.write("Name: ") local n = read() 
            if n~="" and not fs.exists(fs.combine(path, n)) then fs.open(fs.combine(path, n), "w").close() end
        elseif choice == "New Folder" then 
            mainWin.write("Dir: ") local n = read() 
            if n~="" and not fs.exists(fs.combine(path, n)) then fs.makeDir(fs.combine(path, n)) end
        elseif choice == "Delete" then 
            fs.delete(fs.combine(path, file))
        elseif choice == "Rename" then 
            mainWin.write("New: ") local n = read() 
            if n~="" and not fs.exists(fs.combine(path, n)) then 
        ...
        elseif ev == "mouse_click" then
                    ...
                    parallel.waitForAny(
                        ...
                        function()
                            while true do
                                ...
                                if e == "mouse_click" and ty == h then os.queueEvent("mouse_click", 1, tx, ty) return end
                            end
                        end
                    )
                    term.redirect(old)
            ...
            elseif activeTab == "FILE" and y > 1 and y < h then
                ...
                elseif sel then
                    ...
                    else local old = term.redirect(mainWin) shell.run("edit", p) term.redirect(old) drawUI() end
                end
