...
-- 1. ЦВЕТОВАЯ ПАЛИТРА И НАСТРОЙКИ
local themes = {
...
-- Создание виртуальных слоев (Окон)
...
-- 3. АНИМАЦИЯ FUSION (Legacy v32.5)
...
local function showContext(x, y, fileName)
    local opts = fileName and {"Copy", "Delete"} or {"New File", "Paste"}
    ...
    if mx >= x and mx <= x+11 and my >= y and my < y+#opts then
        ...
        elseif choice == "Delete" then
            fs.delete(fs.combine(target, fileName))
        elseif choice == "Copy" then
            clipboard.path = fs.combine(target, fileName)
...
local function updateSystem()
    ...
    mainWin.write(" Cleaning old startup...")
    if fs.exists("startup.lua") then fs.delete("startup.lua") end
    mainWin.setCursorPos(1,2)
    mainWin.write(" Downloading update...")
    ...
    while running do
        ...
        elseif ev == "mouse_click" then
            ...
            if y == h then
                ...
                clockTimer = os.startTimer(0.1) -- Рестарт часов
                drawUI()
            ...
            elseif activeTab == "FILE" and y > 1 and y < h then
                ...
                elseif sel then
                    ...
                    if fs.isDir(p) then currentPath = p else
                        local old = term.redirect(mainWin)
                        term.setCursorBlink(true) shell.run("edit", p)
                        term.setCursorBlink(false) term.redirect(old)
                    end
                end
                drawUI()
            ...
            elseif activeTab == "HOME" and y > 1 and y < h then
                ...
                elseif sel then 
                    ...
                    if fs.isDir(p) then activeTab = "FILE" currentPath = p else
                        local old = term.redirect(mainWin)
                        term.setCursorBlink(true) shell.run("edit", p)
                        term.setCursorBlink(false) term.redirect(old)
                    end
